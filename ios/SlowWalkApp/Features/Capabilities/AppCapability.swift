...
extension CapabilityCatalog {
    /// What this build can really do in the device-first medicine demo.
    ///
    /// Every value here is checked against the source that implements it:
    ///
    /// - `medicineRecognition` — `PresetMedicineTextRecognizer` hands out a
    ///   fixed script. No camera, no image.
    /// - `medicineRiskAssessment` — `LocalMedicineAssessmentRequester` runs
    ///   canonical assessment rules on the device. No server call is made.
    ...
    static let currentDemo = CapabilityCatalog(
        availability: [
        ...
        detail: [
            ...
            .medicineRiskAssessment: "使用设备内演示规则生成提示，不连接服务端，不用于临床判断。",
