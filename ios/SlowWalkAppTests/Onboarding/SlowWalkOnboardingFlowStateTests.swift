...
struct SlowWalkOnboardingFlowStateTests {
    ...
    @Test func itemEntryEnforcesDomainCountAndLengthLimits() {
        ...
        #expect(
            SlowWalkOnboardingInputRules.appending("0", to: fullGroup)
                == .failure(.duplicate)
        )
    }
    ...
    @Test func uiAppendedItemsRemainValidUnderDomainValidator() throws {
        let allergies = try SlowWalkOnboardingInputRules.appending(
            " 花生 ",
            to: ["青霉素", "", "青霉素"]
        ).get()
        ...
        let bundle = try UserProfileDraftValidator().validate(
            draft,
            id: UUID(),
    ...
    @Test func visibleDeleteContractRemovesOnlyTheChosenItem() {
        ...
        #expect(
            SlowWalkOnboardingListMutation.removing(at: 1, from: items)
                == ["高血压", "冠心病"]
        )
        #expect(
            SlowWalkOnboardingListMutation.removing(at: 8, from: items)
                == items
        )
        #expect(
            SlowWalkOnboardingListField.allergies
                .deleteAccessibilityLabel(for: "花生")
                == "删除过敏情况：花生"
        )
    }
    ...
    @Test func textFieldsHavePurposeLabelsHintsAndLiteralFocusOrder() {
        ...
        #expect(fields.map(\.accessibilityLabel) == [
            "称呼", "年龄", "健康情况", "过敏情况", "当前用药",
        ])
