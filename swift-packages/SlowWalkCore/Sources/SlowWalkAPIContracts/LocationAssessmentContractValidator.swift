...
public enum LocationAssessmentValidationFailure: Sendable, Equatable {
    case invalidSample
    case stale
...
/// Canonical field and data-quality validation shared by local and HTTP
/// location assessment adapters.
public struct LocationAssessmentContractValidator: Sendable {
    private let configuration: LocationRiskConfiguration
    private let dataQualityAssessor: any LocationDataQualityAssessing

    public init(
        configuration: LocationRiskConfiguration = .demo,
        dataQualityAssessor: (any LocationDataQualityAssessing)? = nil
    ) {
        self.configuration = configuration
        self.dataQualityAssessor =
            dataQualityAssessor
            ?? LocationDataQualityAssessor(configuration: configuration)
    }
    ...
    public func validateFields(
        ...
        if !request.destination.geofenceRadiusMeters.isFinite
            || request.destination.geofenceRadiusMeters <= 0
        {
            details.append(
                detail(
                    field: "destination.geofenceRadiusMeters",
                    code: "out_of_range",
                    message:
    ...
    public func dataQualityFailure(
    ...
    ) -> LocationAssessmentValidationFailure? {
        ...
        guard samples.count >= configuration.minimumSamplesForAssessment else {
            return .insufficientHistory
        }

        let staleIndices = Set(
            quality.issues.compactMap {
                $0.code == .staleSample ? $0.sampleIndex : nil
            }
        )
        ...
        let currentIndices = Set(samples.indices).subtracting(staleIndices)
